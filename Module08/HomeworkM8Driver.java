...
public class HomeworkM8Driver {
	...
	public static double sortedness(Comparable[] array) {
		...
		return inversionCount / (array.length - 1);
	}
	...
	public static double sortedness(Node<Comparable> node) {
		...
		if (node == null || node.next == null) {
			...
			Node<Comparable> traceNode = node;

			while(traceNode.getNextNode() != null) {
				if (traceNode.getData().compareTo(traceNode.getNextNode().getData()) <= 0) {
					inversionCount++;
				}
